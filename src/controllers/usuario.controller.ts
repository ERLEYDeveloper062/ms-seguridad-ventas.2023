...
export class UsuarioController {
  ...
  async create(
    ...
    //asignar la clave cifradad al usuario
  ...
  /**
   * Metodo personalizados para la API
   */
  ...
  @response(200, {
    description: "Validacion de permisos de usuarios para la logica de negocio",
    content: {'application/json': {schema: getModelSchemaRef(PermisosRolMenu)}}
  })
  async ValidadrPermisosDeUsuario(
    @requestBody(
      {
        content: {
          'application/json': {
      ...
      if (usuario) {
        ...
        try {
          this.usuarioRepository.logins(usuario._id).patch(
            ...
            {
              estadoCodigof2: false
            });
        } catch {
          console.log("No se ha generado el cambio del estado del token en la base de datos.");
        }
        return {
          user: usuario,
          token: token
